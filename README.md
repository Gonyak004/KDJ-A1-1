...
* **사용 구조**: 메인 메뉴 및 서브 메뉴 간 continuous `while True` 무한 루프 반복 구조 적용
* **설계 이유**:
  * **사용자 편의성**: 하나의 작업을 마친 뒤 프로그램이 바로 종료되지 않고, 사용자가 원하는 다른 작업을 계속해서 연속 수행할 수 있도록 대화형 인터페이스(CLI) 환경을 유지합니다.
  * **입력 오류 복구 (Robustness)**: 잘못된 메뉴 번호나 데이터 입력 시 프로그램이 갑자기 튕기거나 비정상 종료되는 대신, 예외 안내 메시지를 출력하고 즉시 재입력을 유도합니다.

* **종료 조건 구현**:
  * **메인 메뉴**: `0` 입력 시 `break` 문을 통해 `while` 루프를 안전하게 빠져나오며 프로그램을 정상 종료합니다.
  * **서브 메뉴 (새 프롬프트 추가, 검색 등)**: 입력 단계에서 `0` 입력 시 `return` 문을 통해 함수 실행을 즉시 취소하고 상위 메인 메뉴 루프로 복귀하도록 설계하여 명확한 이전(Back) 기능을 제공합니다.

---

자료구조 선택 이유 및 장단점 분석 (Data Structures)
...
동명(타이틀 중복) 검사 정책 (Duplicate Title Handling)

* **중복 허용 여부**: **동일한 제목(Title)의 중복 등록을 원칙적으로 금지**합니다.
* **검사 시점**: 신규 프롬프트 추가(`Create`) 및 기존 프롬프트 수정(`Update`) 시점에 즉시 수행됩니다.
* **대소문자 및 공백 처리**:
  * 입력된 제목의 양쪽 공백(`strip()`)을 제거한 뒤 검사합니다.
  * 대소문자 구분을 없앤 소문자 변환(`lower()`) 비교를 통해 `Prompt`와 `prompt` 등의 우회 등록을 방지합니다.
* **예외 처리 흐름**:
  1. 중복된 제목이 발견되면 `"[오류] 이미 존재하는 프롬프트 제목입니다."` 안내 메시지를 출력합니다.
  2. 작업을 즉시 중단하고 **이전 입력 상태로 되돌아가 재입력을 유도**하여 기존 데이터가 유실되지 않도록 보호합니다.

---

고유 식별자(ID) 부여를 통한 데이터 충돌 방지

* **식별자 할당**: 제목이 동일하거나 유사하더라도 시스템 내부에서는 고유한 숫자형 **`ID` (Primary Key)**를 기반으로 모든 CRUD(조회·수정·삭제) 작업을 수행합니다.
* **자동 증가(Auto-Increment) 로직**:
  * 신규 프롬프트 저장 시 `기존 최대 ID + 1` 방식을 통해 ID 중복 발생 가능성을 차단합니다.
  * 데이터 삭제 시 기존 ID를 재사용하지 않아 참조 충돌을 예방합니다.

---

파일 저장 및 동기화 충돌 해결 정책 (Persistence & Sync Policy)

* **단일 원본 데이터(Single Source of Truth)**: 모든 데이터 변경 사항은 메모리에 즉시 반영된 후 `prompts.json` 파일에 즉시 동기화 저장됩니다.
* **JSON 입출력 예외 안전망**:
  * 저장 시 원본 파일 파손을 방지하기 위해 UTF-8 인코딩을 보장하며, JSON 파싱 실패 시 백업 데이터를 유지하도록 로드 에러 처리(`try-except`)를 구획화했습니다.
